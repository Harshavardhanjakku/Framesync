...
export default function MediaPage({ keycloak }) {
  ...
  <div className="flex items-center gap-1 p-1 bg-white/80 backdrop-blur-3xl rounded-2xl border border-blue-200/50">
    {['all', 'image', 'video'].map((type) => (
      ...
      > <span className="relative inline-block">
        ...
        <span
          className={`absolute bottom-[-5px] left-1/2 w-0 h-[2px] bg-blue-500 transition-all duration-300 ease-out transform -translate-x-1/2 ${
            filterType === type
              ? 'w-full'
              : 'group-hover:w-full'
          }`}
