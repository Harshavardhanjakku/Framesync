...
-- Ensure keycloak_id is unique for ON CONFLICT in app logic
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE schemaname = 'public' AND indexname = 'users_keycloak_id_unique'
  ) THEN
    -- Create a unique index (safer than altering table if existing data)
...
-- Add optional mapping to Keycloak organization id if missing
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'organizations' AND column_name = 'keycloak_org_id'
  ) THEN
